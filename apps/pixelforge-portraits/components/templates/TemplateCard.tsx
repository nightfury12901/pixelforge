...
export function TemplateCard({ template }: TemplateCardProps) {
  return (
    /*
     * content-visibility: auto — browser completely skips rendering
     * this card when it's off-screen, which is the #1 scroll perf fix.
     * contain-intrinsic-size gives the browser a placeholder size so
     * the scrollbar stays accurate while items are skipped.
     */
    <Link
      href={`/dashboard/generate/${template.id}`}
      className="block"
      style={{
    ...
    >
      <div
        className="group relative cursor-pointer rounded-2xl overflow-hidden bg-gray-900/50
