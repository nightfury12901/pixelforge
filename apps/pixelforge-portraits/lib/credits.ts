...
export async function deductCredits(
  ...
  if (profile.credits_remaining < creditsToDeduct) {
    return {
      success: false,
      remaining: profile.credits_remaining,
      error: 'No credits remaining. Please upgrade your pack.',
    };
  }

  // Handle free operations gracefully without hitting the database for a 0 update
  ...
  const { error: updateError } = await db
    .from('profiles')
    .update({ credits_remaining: Math.max(0, profile.credits_remaining - creditsToDeduct) })
    .eq('id', userId);
...
export async function resetCreditsForTier(
  ...
  const creditAmounts = { free: 0, starter: 5, creator: 12, pro: 25 };

  const { error } = await db
    .from('profiles')
    .update({
      credits_remaining: creditAmounts[tier],
      credits_reset_date: new Date(Date.now() + 3650 * 24 * 60 * 60 * 1000).toISOString(), // "Lifetime" essentially
    })
    .eq('id', userId);
